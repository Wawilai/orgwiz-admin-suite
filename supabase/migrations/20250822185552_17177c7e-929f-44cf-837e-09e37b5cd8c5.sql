-- =============================
-- COMPREHENSIVE DATABASE SCHEMA FOR ADMIN MANAGEMENT SYSTEM (Fixed)
...
-- 7. Mailboxes (Fixed computed column)
CREATE TABLE public.mailboxes (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    domain_id UUID NOT NULL REFERENCES public.domains(id) ON DELETE CASCADE,
    user_id UUID REFERENCES public.profiles(user_id) ON DELETE SET NULL,
    local_part TEXT NOT NULL,
    domain_name TEXT, -- Store domain name directly for performance
    display_name TEXT,
    password_hash TEXT,
...
-- Create function to get full email address
CREATE OR REPLACE FUNCTION public.get_full_email_address(local_part TEXT, domain_id UUID)
RETURNS TEXT AS $$
BEGIN
    RETURN local_part || '@' || (SELECT name FROM public.domains WHERE id = domain_id);
END;
