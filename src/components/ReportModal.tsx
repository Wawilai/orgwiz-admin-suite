...
export default function ReportModal({
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold">
              {previewData.license.overview.total.toLocaleString()}
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-orange-600">
              {previewData.license.overview.utilization}%
            </div>
            <div className="text-xs text-muted-foreground">อัตราการใช้งาน</div>
            <Progress
              value={previewData.license.overview.utilization}
              className="mt-2"
            />
          </CardContent>
        </Card>
        ...
        >
          ...
          <div className="text-right">
            <div className="text-sm font-medium">
              {service.users.toLocaleString()} ผู้ใช้
            </div>
            <div className="text-xs text-muted-foreground">
              {service.actions.toLocaleString()} การใช้งาน
            </div>
          </div>
        </div>
      ))}
  ...
  const renderMailRelayPreview = () => (
    <div className="space-y-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-green-600">
              {previewData["mail-relay"].stats.accepted.toLocaleString()}
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-blue-600">
              {previewData["mail-relay"].stats.successRate}%
            </div>
            <div className="text-xs text-muted-foreground">Success Rate</div>
            <Progress
              value={previewData["mail-relay"].stats.successRate}
              className="mt-2"
            />
          </CardContent>
        </Card>
      ...
      <div className="p-3 border rounded bg-green-50">
        ...
        <div className="text-xs text-muted-foreground">
          {previewData["mail-relay"].systems} ระบบทำงานปกติ
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        {[
          {
            label: "> 30 วัน",
            value: previewData.inactive.counts.days30,
            color: "text-orange-600",
          },
          {
            label: "> 90 วัน",
            value: previewData.inactive.counts.days90,
            color: "text-red-600",
          },
        ].map((item, index) => (
          <Card key={index}>
      ...
      <div className="p-3 border rounded bg-yellow-50">
        ...
        <div className="text-xs text-muted-foreground">
          {previewData.inactive.recommendations} ข้อเสนอแนะที่ต้องดำเนินการ
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold">
              {previewData.contacts.overview.orgs}
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-green-600">
              {previewData.contacts.overview.coverage}%
            </div>
      ...
      <div className="p-3 border rounded bg-blue-50">
        ...
        <div className="text-xs text-muted-foreground">
          {previewData.contacts.overview.admins} คน (เฉลี่ย{" "}
          {previewData.contacts.overview.avgPerOrg} คน/องค์กร)
        </div>
      </div>
    </div>
  );
  ...
  const renderPreviewContent = () => {
    switch (reportType) {
      ...
      case "license":
        return renderLicensePreview();
      case "services":
        return renderServicesPreview();
      case "mail-relay":
        return renderMailRelayPreview();
      case "inactive":
        return renderInactivePreview();
      case "contacts":
        return renderContactsPreview();
      default:
        return <div>ไม่พบข้อมูลตัวอย่าง</div>;
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>รายงานและส่งออกข้อมูล</DialogDescription>
        </DialogHeader>
